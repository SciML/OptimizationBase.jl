...
function OptimizationBase.instantiate_function(f, x, adtype::AutoSparseFiniteDiff, p,
        num_cons = 0)
    if maximum(getfield.(methods(f.f), :nargs)) > 3
        error("$(string(adtype)) with SparseDiffTools does not support functions with more than 2 arguments")
    end
...
function OptimizationBase.instantiate_function(f, x, adtype::AutoSparseReverseDiff,
