...
@testset "StaticArray" begin
    using StaticArraysCore, StaticArrays
    x0 = @SArray [0.0, 0.0]
    ...
    @test optprob.grad(x0) == SVector{2}(G1)
    @test optprob.hess(x0) == SArray{(2,2)}(H1)
    @test optprob.cons(x0) == @SArray [0.0, 0.0]
    @test optprob.cons_j([5.0, 3.0])≈ @SArray [10.0 6.0; -0.149013 -0.958924] rtol=1e-6
    @test optprob.cons_h(x0) == @SArray [[2.0 0.0; 0.0 2.0], [-0.0 1.0; 1.0 0.0]]
    ...
    @test optprob.grad(x0) == @SArray G1
    @test optprob.hess(x0) == @SArray H1
    @test optprob.cons(x0) == @SArray [0.0, 0.0]
    @test optprob.cons_j([5.0, 3.0])≈ @SArray [10.0 6.0; -0.149013 -0.958924] rtol=1e-6
    @test optprob.cons_h(x0) == @SArray [[2.0 0.0; 0.0 2.0], [-0.0 1.0; 1.0 0.0]]
    ...
    @test optprob.grad(x0) == @SArray G1
    @test Array(optprob.hess(x0)) ≈ @SArray H1
    @test optprob.cons(x0) == @SArray [0.0, 0.0]
    @test optprob.cons_j([5.0, 3.0])≈ @SArray [10.0 6.0; -0.149013 -0.958924] rtol=1e-6
    @test Array.(optprob.cons_h(x0)) ≈ @SArray [[2.0 0.0; 0.0 2.0], [-0.0 1.0; 1.0 0.0]]
